...
def test_crud_stateless_flow(protocol):
    with WolfFlow(
        filepath=os.path.join(cur_dir, 'flows', f'flow-{protocol}-stateless.yml'),
...
@pytest.mark.parametrize('protocol', ['http', 'grpc'])
def test_crud_http_flow_stateful(protocol):
    # This tests
    ...
    with WolfFlow(filepath=FLOW_FILE_PATH, name=INDEX_FLOW_NAME) as index_flow:
    ...
    with WolfFlow(
        filepath=FLOW_FILE_PATH, name=SEARCH_FLOW_NAME, workspace=index_flow.workspace
    ) as search_flow:
