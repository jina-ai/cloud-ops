...
sleep $SLEEP_TIME && \
$PYTHON_EXEC client.py --dataset $DATASET -t index -n 100 -l $TIME_LOAD_INDEX -c $CONCURRENCY -r $REQ_SIZE && \
...
sleep $SLEEP_TIME && \
$PYTHON_EXEC client.py --dataset $DATASET -t query -n 10 -l $TIME_LOAD_QUERY -c $CONCURRENCY -r $REQ_SIZE && \
sleep $SLEEP_TIME && \
export flow=`cat flow.txt` && \
$PYTHON_EXEC app.py --jinad remove --flow-id $flow && \
sleep $SLEEP_TIME && \
